...
# Reflexion
mat_reflect = np.matmul (np.array([[1,0,0],[0,-1,0],[0,0,1]])  , np.array([[1,0,0],[0,1,-h],[0,0,1]]))
...
# Rotate
theta = np.pi/6
mat_rotate = np.matmul( np.array([[1,0,w/2],[0,1,h/2],[0,0,1]]) , np.array([[np.cos(theta),np.sin(theta),0],[np.sin(theta),-np.cos(theta),0],[0,0,1]]) )
mat_rotate = np.matmul( mat_rotate, np.array([[1,0,-w/2],[0,1,-h/2],[0,0,1]]))
...
mat_all = np.matmul(mat_reflect, mat_scale)  
mat_all = np.matmul(mat_all, mat_rotate)
mat_all = np.matmul(mat_all, mat_shear)
